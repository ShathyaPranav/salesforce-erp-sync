...
| Backoff base and cap are settings (1 s locally, 60 s on AWS, cap 15 min) | Keeps the 100%-failure test fast. With 5 receives the delays are 60, 120, 240 and 480 s, a retry budget of about 15 minutes, so a longer ERP outage lands in the DLQ and is recovered by `relayctl dlq redrive` or the reconciler |
...
| The reconciler's core (compare and re-enqueue) is built in Phase 1.4; Phase 2.3 schedules it on AWS and adds `relayctl reconcile` | Phase 1.4 needs a passing test for every row of the failure table, and two rows ("poller misses a record", "deal reverted or deleted") are the reconciler's job |
